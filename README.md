...
| `SUPER + S` | スクラッチパッド |
| `SUPER + M` | メール用スクラッチパッド（Thunderbird の出し入れ） |
| `SUPER + SHIFT + M` | ウィンドウをメール用スクラッチパッドへ送る |
...
タイル型ウィンドウマネージャには最小化がありません。代わりに Hyprland の
**special workspace（スクラッチパッド）** を使います。ウィンドウは動いたまま画面から
消えるので、実質的に最小化と同じです。

Thunderbird は**ログイン時に自動起動し、そのままメール用スクラッチパッドに入ります**
（`silent` 指定なので画面は切り替わりません）。未読数は Thunderbird が同期して初めて
更新されるので、常時動かしておくほうが Birdtray とも噛み合います。

| キー | 動作 |
| --- | --- |
| `SUPER + M` | メールの表示 / 非表示 |
| `SUPER + SHIFT + M` | いま選んでいるウィンドウをメール用スクラッチパッドへ送る |
| `SUPER + S` / `SUPER + SHIFT + S` | 汎用スクラッチパッド（`magic`）の表示 / 送る |

Birdtray のアイコンをクリックして Thunderbird を隠す機能（`showhidethunderbird`）は
**Wayland では効きません**。Birdtray のウィンドウ制御は X11 の API に依存していて、
Wayland ネイティブのウィンドウを掴めないためです。ここは `SUPER + M` を使ってください。

Thunderbird を自動起動したくない場合は、`hyprland.lua` の自動起動から
`thunderbird` の行を消してください。手で起動しても、ウィンドウルールで
スクラッチパッドに入ります。
